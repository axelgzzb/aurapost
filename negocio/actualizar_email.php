...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    } elseif ($u->emailExists($valor, $id)) {
        $errores[] = "Este email ya está en uso por otra cuenta.";
        $_SESSION["error"]= "Este email ya está en uso por otra cuenta.";
    } else {
        $datosActualizar = ['email' => $valor];
        
        if ($u->updateUser($id, $datosActualizar)) {
            $exito = true;
            $_SESSION["usuario"]["email"] = $valor;
            $datos["email"] = $valor;
        } else {
            $errores[] = "Error al actualizar el email.";
...
<head>
    ...
    <title>Actualizar Email - AuraPost</title>
...
<main class="container main-container py-4">
    ...
    <div class="text-center mb-5">
        <h1 class="text-gradient" style="font-size: 2.5rem; font-weight: 700;">
            <i class="bi bi-envelope-fill me-3"></i>Actualizar Email
        </h1>
        <p class="text-muted">Modifica tu correo electrónico</p>
    </div>
            ...
            <form method="POST" action="">
                ...
                <div class="mb-4">
                    ...
                    <input type="email" 
                           ...
                           required>
                    <small class="text-muted">Este email se usará para iniciar sesión en tu cuenta</small>
                </div>
